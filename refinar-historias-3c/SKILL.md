---
name: refinar-historias-3c
description: Use quando uma história de usuário precisa ser refinada por Card, Conversation e Confirmation, especialmente ao preparar Description e Acceptance Criteria para o Azure Boards ou quando um cartão está sendo tratado como requisito completo.
---

# Refinar histórias com 3C
...
2. **Card — cartão.** **REQUIRED SUB-SKILL:** use refinar-historias-3w. O cartão é um lembrete conciso e negociável, não uma especificação. Registre história ou rascunho, contexto essencial e estado 3W. Se houver evidência Brownfield, forneça-a somente como contexto rotulado; ela não resolve Who, What ou Why.
   Se a 3W sinalizar um débito técnico, **REQUIRED OPTIONAL SUB-SKILL:** use
   `especificar-debitos-tecnicos` com o sinal, a evidência e o contexto da demanda.
   Mantenha a spec de débito separada do Card e prossiga com o refinamento da história.
...
indique `revisar-textos-requisitos` como revisão manual opcional. A revisão de copy pode revelar
...
- A chamada opcional de `especificar-debitos-tecnicos` não substitui os gates 3C nem
  autoriza criar ou alterar work items; devolva a spec de débito ao chamador como artefato separado.
