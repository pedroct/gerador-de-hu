...
- Em Brownfield, `Validation Summary` contém a matriz definida em [brownfield-validation.md](brownfield-validation.md), uma linha para cada requisito da spec, e resume quais requisitos originaram trabalho, permaneceram apenas na cobertura ou não puderam ser validados. O status de cada linha também sustenta a decisão de tipo (User Story ou Bug) do item de folha correspondente.
...
- Em User Story ou Bug: contém, nesta ordem e exatamente uma vez, os headings `###### Card` e `###### Conversation`. Sob cada heading, transcreva o conteúdo correspondente retornado pela `refining-user-stories-with-3c`; não recalcule nem reestruture o conteúdo.
...
`Refinement Status` é metadado do documento de preparação, fora dos campos do Azure Boards. Ele registra os estados devolvidos pela 3C e a origem do item de folha; não é anexado automaticamente a `Description` nem a `Acceptance Criteria`.
