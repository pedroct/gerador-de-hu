"""Valida a estrutura de um backlog Markdown do Azure Boards.

Esta é a cópia **canônica** das regras de contrato: os `contrato_backlog.py` das duas
publicadoras a seguem. O trecho entre os marcadores abaixo é espelho mecânico de
`publicar-backlog-azure-boards/src/publicar_backlog_azure_boards/contrato_backlog.py`
(sem a linha de import do pacote), e `tests/test_sincronia_do_contrato.py` compara os dois
byte a byte. Não edite o trecho à mão: mude o contrato e reespelhe, ou o gate que o
`SKILL.md` manda rodar passa a recusar exatamente o que o `SKILL.md` manda emitir.
"""
...
# --- início do trecho espelhado de contrato_backlog.py ---
ITEM_RE = re.compile(
...
# --- fim do trecho espelhado de contrato_backlog.py ---
