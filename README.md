...
- **Drafting a partir de Demanda no Azure Boards:** lê por ID, somente com `GET`, uma Demanda de Negócio já registrada, valida o tipo e o contrato de campos, e produz a spec rastreável a cada campo remoto, orquestrando as análises de débitos técnicos, telas UX-UI e copy quando seus gatilhos existirem, e grava tudo numa pasta `DN-<id>-<slug>/` sob `docs/specs/`, com nomes de arquivo de vocabulário fechado.
